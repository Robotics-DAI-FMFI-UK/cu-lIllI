...
#define DEFAULT_CALIB_RIGHT_HAND_CLOSING         420
...
int liveControlStatus = 0;   //0 - not receiving message; 1 - awaiting servo number; 2 - awaiting first byte; 3 - awaiting second byte
int liveControlServo = -1;
byte liveControlServoByte1;
byte liveControlServoByte2;
...
void console_loop()
{
  ...
  {
    char c = Serial.read();
    if (liveControlStatus != 0) {
      switch (liveControlStatus) {
        case 1:
          liveControlServo = c;
          // Serial.println(F("l1"));
          ...
          Serial.print(F("po: "));
          Serial.println(liveControlStatus);
          liveControlStatus = 2;
          Serial.print(F("pred: "));
          Serial.println(liveControlStatus);
          ...
          break;
        case 2:
          liveControlServoByte1 = c;
          liveControlStatus = 3;
          Serial.println(F("l2"));
          break;
        case 3:
          liveControlServoByte2 = c;
          uint16_t liveControlServoPos = 0;
          liveControlServoPos = (liveControlServoByte1 << 8) + liveControlServoByte2;
          int liveControlServoNumber = default_position_servos[liveControlServo];

          Serial.println(F("l3"));
          Serial.print(F("liveControlServoPos: "));
          Serial.println(liveControlServoPos);
          Serial.print(F("liveControlServoNumber: "));
          Serial.println(liveControlServoNumber);
          liveControlStatus = 0;
          // return;

          if (liveControlServoNumber < 16)
              left.setPWM(liveControlServoNumber, 0, liveControlServoPos);
            else
              right.setPWM(liveControlServoNumber - 16, 0, liveControlServoPos);
          break;
      }
      return;
    }
    ...
    if (direct >= 0) {
      ...
      switch (c) {
        ...
        case '$':
                  liveControlStatus = 1;
                  Serial.print(F("aktivovany mod liveControl"));
                  break;
